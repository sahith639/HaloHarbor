...
def federated_training(global_model):
    ...
    with open('./global_update.pkl', 'rb') as file:
        ...
        response = requests.post(url, json={'value': str(pickle.dumps(
            loaded_data), 'latin1'), 'data': {'client_id': 0, 'epochs': 3}})
        ...
        if response.status_code == 200:
            ...
            print("Error occurred while sending file to API. Status code:",
...
def resopnse():
    ...
    if all(i == 1 for i in updated):
        ...
        if rounds_counter <= num_rounds:
            ...
            with open('./global_update.pkl', 'rb') as file:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        "Error occurred while sending file to API. Status code:", response.status_code)
        else:
            print('All Done!')
        ...
        with open('./global_update.pkl', 'rb') as file:
            ...
            headers = {'Content-Type': ': application/json'}
            response = requests.post(url, json={'value': str(pickle.dumps(
                loaded_data), 'latin1'), 'data': {'client_id': int(data["client_id"])+1, 'epochs': 3}})
            if response.status_code == 200:
                print("File successfully sent to API.")
            else:
                print(
