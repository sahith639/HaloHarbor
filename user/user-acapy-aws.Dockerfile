...
CMD aca-py start --wallet-storage-type postgres_storage --wallet-storage-config "$WALLET_STORAGE_CONFIG" --wallet-storage-creds '{"account":"testuser","password":"testpassword","admin_account":"testuser","admin_password":"testpassword"}' --inbound-transport http 0.0.0.0 8030 --outbound-transport http --endpoint "$ENDPOINT_URL" --webhook-url "$WEBHOOK_URL" --genesis-url 'http://test.bcovrin.vonx.io/genesis' --auto-accept-invites --auto-accept-requests --auto-ping-connection --auto-respond-credential-proposal --auto-respond-credential-offer --auto-respond-credential-request --auto-verify-presentation --auto-store-credential --label user.agent --log-level 'debug' --admin-insecure-mode --admin 0.0.0.0 8031 --auto-provision --wallet-type askar --wallet-name user-wallet --wallet-key wallet-password
